...
class PlayerStatistics
{
	...
	private $stat_fields = array('lastName'		=> "",
															 'firstName'		=> "",
															 'nonvenue'		=> 0,
															 'venue_wins'	=> 0,
															 'distinct_venues' => 0,
	...
	public $VenueScores = array(array());
	private $_VenueTitles = array();
		...
		if($query->num_rows > 0)
		{
			while($data = $query->fetch_assoc())
			{
				if($data['venue_wins'] > 0) 
				{
					...
					if(isset($this->VenueScores[$data['month']-1][$data['venue_id'] . "|" . date("N", strtotime($data['year']."-".$data['month']."-".$data['day']))]))
						$this->VenueScores[$data['month']-1][$data['venue_id'] . "|" . date("N", strtotime($data['year']."-".$data['month']."-".$data['day']))] += $data['venue_wins'];
					elseif(isset($this->VenueScores[$data['month']-1][$data['venue_id']]))
						$this->VenueScores[$data['month']-1][$data['venue_id']] += $data['venue_wins'];
				}
	...
	{
		$this->_VenueTitles = Venue::getSeperatedVenues();
		for($i=0;$i<12;++$i)
		{
			$this->Statistic[$i] = $this->stat_fields;
			foreach($this->_VenueTitles as $k => $v)
			{
				$this->VenueScores[$i][$k] = 0;
			}
		}
	}
	...
	public function outputLeaderboardRow( $m = "" )
	{
		...
		for($mk = 0; $mk < 12; ++$mk)
		{
			$this->Statistic[$mk]['lastName'] = CMS_Core::getInstance()->getUserLast( $this->pid );
			$this->Statistic[$mk]['firstName'] = CMS_Core::getInstance()->getUserFirst( $this->pid );
		}
		foreach($this->VenueScores[$m] as $v)
		{
			$venue_cols .= $Template->Skin['leaderboard']->leaderboardVenueCol($v);
		}
		$HTML = $Template->Skin['leaderboard']->searchFullRow( $this->Statistic[$m], $venue_cols );
		return $HTML;
	}
	
	public function getCSVRow( $m = "" )
	{
		...
		$statistics = array("lastname" => $this->Statistic[$m]['lastName'],
												"firstname" => $this->Statistic[$m]['firstName'],
												"nonvenues" => $this->Statistic[$m]['nonvenue'],
												"venues" => $this->Statistic[$m]['distinct_venues'],
												"vnvbonus" => $this->Statistic[$m]['chip_bonus_vnv'],
												"winbonus" => $this->Statistic[$m]['chip_win_bonus'],
												"bounty" => $this->Statistic[$m]['bounty'],
												"wins" => $this->Statistic[$m]['total_wins'],
												"hh" => $this->Statistic[$m]['high_hand'],
												"xtra" => $this->Statistic[$m]['xtra_points'],
												"monthlytotal" => $this->Statistic[$m]['chip_total'],
												"numgames" => $this->Statistic[$m]['hands_played'],
												"evtpts" => $this->Statistic[$m]['event_points'],
												"monthpts" => $this->Statistic[$m]['point_total'],
												"prevmonpts" => $this->Statistic[$m-1]['point_total'],
												"ytdtotal" => $this->Statistic[$m]['points_ytd'],);
		array_splice( $statistics, 2, 0, $this->VenueScores[$m] ); // splice in at position 2
		
		return $statistics;
	}
	
	public function getVenueHeaders()
	{
		return $this->_VenueTitles;
	}
