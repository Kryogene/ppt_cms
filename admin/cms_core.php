...
class CMS_Core
{
	...
	public function getUserLast( $id )
	{
		...
		$query = $SQL->query("SELECT `lastName` FROM `players` WHERE `id` = '" . $SQL->escape_string( $id ) . "'");
		if($query)
			if($query->num_rows > 0)
			{
				$results = $query->fetch_assoc();
	...
	public function getUserFirst( $id )
	{
		...
		$query = $SQL->query("SELECT `firstName` FROM `players` WHERE `id` = '" . $SQL->escape_string( $id ) . "'");
		if($query)
			if($query->num_rows > 0)
			{
				$results = $query->fetch_assoc();
...
HTML;
	...
	private function _setBody()
	{
		...
		if(is_a( $this->User, "Administrator" ))
			$body .= $Template->Skin['globals']->mainNav( $Template->getMainNavElements() );
	...
	private function _setHeader()
	{
		...
		$header .= $Template->Skin['globals']->Link("stylesheet", "text/css", "default.css?v=1") . "\n";
